...
contract('token contract', function(accounts) {
    ...
    // it("2.2 add admin from accounts[1]", function() {
    //     return tokenContract.addSuperAdmin(accounts[3], {from:accounts[8]});
    // });


    //1536716194
    it("3.getCurrent Blocktime.", function() {
        return tokenContract.getCurrentTime().then(function(result){
            console.log("log : " + result.valueOf());
        })
    });

    it("4. setTimelock, accounts[8]", function() {
        return tokenContract.lockAccount(accounts[8], 2536716194, {from:accounts[1]});
    });

    it("4.1 transfer from non owner when transfers started, account[8]", function() {
        var value = new BigNumber(5);
        return tokenContract.transfer(accounts[4], value, {from:accounts[8]}).then(function(result){
    ...
    it("4.2 approve and transfer test from accounts[8](timelocked.)", function() {
        var value = new BigNumber(5);
        return tokenContract.approve(accounts[3], value, {from:accounts[8]}).then(function(){
            tokenContract.transferFrom(accounts[8], accounts[3],value,{from:accounts[3]});
    ...
    it("5. setTimelock, accounts[8]", function() {
        return tokenContract.lockAccount(accounts[8], 1536716194, {from:accounts[1]});
    });

    it("5.1 approve and transfer test from accounts[8](timeunlocked.)", function() {
        var value = new BigNumber(5);
        return tokenContract.approve(accounts[3], value, {from:accounts[8]}).then(function(){
            tokenContract.transferFrom(accounts[8], accounts[2],value,{from:accounts[3]});
    ...
    // it("transfer from unfrozen account to frozen account", function() {
    //     var value = new BigNumber(500);
    ...
    // it("approve from frozen account to frozen account.", function() {
    //     var value = new BigNumber(5);
    ...
    // it("approve from unfreezed account to freezed account.", function() {
    //     var value = new BigNumber(5);
